        ...
        namespace pdpd
        {
            namespace op
            {
                NamedOutputs range(const NodeContext& node)
                {
                    ...
                    PDPD_ASSERT(type[0] == element::i64 || type[0] == element::i32 || type[0] == element::f32,
                    "Only supports int32, int64, float32");

                    const auto axis = ngraph::opset6::Constant::create(element::i64, Shape{}, {0});
                    auto start_scalar =
                        std::make_shared<ngraph::opset6::Squeeze>(start, axis);
                    auto stop_scalar =
                        std::make_shared<ngraph::opset6::Squeeze>(stop, axis);
                    auto step_scalar =
                        std::make_shared<ngraph::opset6::Squeeze>(step, axis);
