...
class TestMulticlassNms(LayerTest):
    def test_multiclass_nms(self):
        boxes_np = np.random.rand(10, 50, 4).astype('float')
        scores_np = np.random.rand(10, 81, 50).astype('float')
        with self.static_graph():
            boxes = paddle.static.data(
                name='bboxes', shape=[10, 50, 4], dtype='float')
            scores = paddle.static.data(
                name='scores', shape=[10, 81, 50], dtype='float')
