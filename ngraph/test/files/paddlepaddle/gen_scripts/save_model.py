...
def saveModel(name, exe, feedkeys: list, fetchlist: list, inputs: list, outputs: list, target_dir: str, quite=True):
    ...
    for i, input in enumerate(inputs):
        ...
        np.save(os.path.join(model_dir, "input{}.{}.{}".format(
            i, feedkeys[i], input.dtype)), input)
    print("\n")
    ...
    pdpd.fluid.io.save_inference_model(
        model_dir, feedkeys, fetchlist, exe, model_filename=name+".pdmodel", params_filename=name+".pdiparams")
