...
ov::intel_cpu::AUGRUSequenceNode::AUGRUSequenceNode(const Output<Node>& X, // [batch_size, seq_length, input_size]
                const Output<Node>& H_t,  // [batch_size, num_directions, hidden_size]
                const Output<Node>& A,  // [batch_size, seq_length, 1]
                const Output<Node>& sequence_lengths, // [batch_size]
                const Output<Node>& W,
                const Output<Node>& R,
                const Output<Node>& B,
                size_t hidden_size,
    ...
    : RNNCellBase({X, H_t, A, sequence_lengths, W, R, B},
                hidden_size,
                clip,
                activations,
                activations_alpha,
...
std::shared_ptr<ngraph::Node> ov::intel_cpu::AUGRUSequenceNode::clone_with_new_inputs(const ngraph::OutputVector& new_args) const {
    INTERNAL_OP_SCOPE(AUGRUSequenceNode_clone_with_new_inputs);
    check_new_args_count(this, new_args);
    return make_shared<ov::intel_cpu::AUGRUSequenceNode>(new_args.at(0), new_args.at(1), new_args.at(2), new_args.at(3), new_args.at(4), new_args.at(5), new_args.at(6),
                                      m_hidden_size, m_direction, m_activations, m_activations_alpha, m_activations_beta, m_clip,
                                      m_linear_before_reset);
}

void ov::intel_cpu::AUGRUSequenceNode::validate_and_infer_types() {
    INTERNAL_OP_SCOPE(AUGRUSequenceNode_validate_and_infer_types);
    ...
    auto sl_pshape = get_input_partial_shape(3);
    auto w_pshape = get_input_partial_shape(4);
    auto r_pshape = get_input_partial_shape(5);
    auto b_pshape = get_input_partial_shape(6);
...
bool ov::intel_cpu::AUGRUSequenceNode::visit_attributes(ngraph::AttributeVisitor &visitor) {
    INTERNAL_OP_SCOPE(AUGRUSequenceNode_visit_attributes);
