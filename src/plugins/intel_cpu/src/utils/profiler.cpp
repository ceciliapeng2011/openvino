...
struct linux_perf_event {
    ...
    linux_perf_event(uint32_t counter, int uid) : config(counter), fd(-1), buf(nullptr), uid(uid) {
        struct perf_event_attr attr = {};
        attr.type = config > 10 ? PERF_TYPE_RAW : PERF_TYPE_HARDWARE;
    ...
    void call(const std::string& inst, States& s, double& var) override {
        if (inst == "pmu") {
            ...
            if (init_mode) {
                ...
                if (events.count(config) == 0) {
                    printf("pmu[%d] config=0x%x \n", event_uid, config);
                    events.emplace(config, std::make_shared<linux_perf_event>(config, event_uid));
                    event_uid++;
                }
                s.stack.back() = 1;
            } else {
                // get current value
                s.stack.back() = rt_counters[events[config]->uid];
            }
            return;
        }
        if (inst == "duration") {
...
void ProfilerManager::addCounter(uint64_t time) {
...
ProfilerManager::ProfilerManager() {
    ...
    if (num_hint > 1) {
        all_data.reserve(num_hint);
#ifdef HW_PERF_COUNTER
        all_counters.reserve(num_hint * 2);
#endif
